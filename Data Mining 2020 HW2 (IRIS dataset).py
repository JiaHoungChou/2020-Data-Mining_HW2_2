...
Species_dataframe= one_hot_encoding(database, "Species")

database= replace_one_hot_variable(dataframe_type= database, index_name= "Species", new_dataframe= Species_dataframe)
database= shuffle(database).drop(["Id"], axis= 1)
...
W_xh_, B_xh_, W_hy_, B_hy_= forword_Backword_computation(X= X_train, label= y_train, eta= 0.01, hidden_size= 5, iteration= 200)
...
prediction= sigmoid(sigmoid(X_test* W_xh_.T- B_xh_)* W_hy_.T- B_hy_)
